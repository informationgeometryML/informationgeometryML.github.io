...
When a K-[dimensional]({{ site.baseurl }}{% post_url 2021-09-06-Geomopt01 %}#dimensionality-of-a-manifold) manifold is not flat such as `$\mathcal{M}\neq \mathcal{R}^K$`,
the parameter space `$\Omega_\tau$` under intrinsic parameterization `$\tau$` is a subspace of `$\mathcal{R}^K$` such as 
 `$\Omega_{\tau}  \subset \mathcal{R}^K$` while the vector space `$T_{\tau_0} (\Omega_\tau)$` is the whole space  `$\mathcal{R}^K$` (see the following figure as an illustration)
</div>
