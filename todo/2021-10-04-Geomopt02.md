...
`$\tau_1$`, which involves the concept of [parallel transport](https://en.wikipedia.org/wiki/Parallel_transport) in a curved space. For simplicity, we aviod defining it since it involves derivatives of the Fisher-Rao metric (as known as the [Christoffel symbols](
