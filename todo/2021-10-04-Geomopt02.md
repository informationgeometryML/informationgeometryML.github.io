...
* When K-dim manifold `$\mathcal{M}\neq \mathcal{R}^K$`,  `$T_{\tau_0} (\mathcal{M}) \neq \mathcal{R}^K$`.  
* By choosing an intrinsic parameterization `$\tau$` in its parameter space `$\Omega_\tau$`, we will see that
 `$T_{\tau_0} (\Omega_\tau) \subset \mathcal{R}^K$`.
</div>
...
`$$
\begin{aligned}
\min_{ \color{red} {\|v\|_{F}^2=1} \\ v \in T_{\tau_0} (\Omega_\tau) } ( \nabla_\tau f(\mathbf{\tau}_0) )^T  \mathbf{v} 
...
* Each possible (Riemannian) vector `$\mathbf{v}$` lives in the same (tangent) vector space at current point `$\mathbf{\tau}_0$` denoted by `$T_{\tau_0} (\Omega_\tau)$`
* `$T_{\tau_0} (\Omega_\tau) = \mathcal{R}^K$`, where `$K$` is the number of entries of the intrinsic parameter array `$\tau$`.
* `$ \Omega_\tau \subset T_{\tau_0} (\Omega_\tau) $` (see the figure)
</div>
