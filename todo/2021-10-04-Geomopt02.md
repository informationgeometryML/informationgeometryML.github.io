...
# Motivation
In machine learning, a common derivation of natural-gradients is via a Talyor expansion of the KL divergence as we will
discussed  in <!--[Part IV]({{ site.baseurl }}{% post_url 2021-11-15-Geomopt04 %}#proximal-gradient-descent).-->
Part IV.  However, such derivation has its limitations:
* It does not clearly illustrate the difference between a parameter space and a (natural) gradient space.
* It implicitly assumes that the FIM is non-singular and the parameter space is open.
...
In Eucclidean cases, there is no difference between parameter space `$\mathcal{R}^K$` and gradient space `$T_{\tau_0} (\mathcal{R}^K)=\mathcal{R}^K$` at point `$\tau_0$`.
...
In K-[dimensional]({{ site.baseurl }}{% post_url 2021-09-06-Geomopt01 %}#dimensionality-of-a-manifold) manifold cases, an intrinsic parameter space `$\Omega_\tau$` is often a **subspace** of `$\mathcal{R}^K$` while the gradient space  `$T_{\tau_0} (\Omega_\tau)$` is the whole space `$\mathcal{R}^K$` illustrated by the following figure.
...
We will discuss more about this in
<!--[Part IV]({{ site.baseurl }}{ post_url 2021-11-15-Geomopt04 }#two-kinds-of-spaces).-->
Part IV. 
