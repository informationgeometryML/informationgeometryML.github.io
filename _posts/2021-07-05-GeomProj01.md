...
About this work {% cite lin2021tractable %} {%  cite lin2021structured %}: [(Youtube) talk](https://www.youtube.com/watch?v=vEY1ZxDJX8o&t=11s), [ICML paper](https://arxiv.org/abs/2102.07405), [workshop paper](https://arxiv.org/abs/2107.10884),
...
`$$ 
\begin{aligned} 
\lambda_{t+1} & \leftarrow  \phi_{\lambda_t} \big( \overbrace{\eta_0-\beta \hat{\mathbf{g}}_{\eta_0}^{(t)} }^{ \text{NGD in local space} } \big) \,\,\,\, \textrm{(update in auxiliary space)}\\
\tau_{t+1} & \leftarrow \psi\big( \lambda_{t+1} \big) \,\,\,\, \textrm{(update in  global space)}
\end{aligned} 
$$`
</fieldset>
...
= \underbrace{  \color{green}{\Big(\begin{array}{cc} \mathbf{I}_p & 0 \\ 0 & 2 \mathbf{I}_{p^2} \end{array} \Big)^{-1}}  }_{ \text{inverse exact FIM } } \Big[\begin{array}{c}  \mathbf{B}_t^{-1} \mathbf{g}_{\mu_t} \\ \mathrm{vec}( -2\mathbf{B}_t^{-1} \mathbf{g}_{\Sigma_t} \mathbf{B}_t^{-T}) \end{array} \Big]
\end{aligned}
$$`
...
Use our NGD to solve `$\eqref{6}$` with $\mathbf{S}=\mathbf{B}\mathbf{B}^T$
...
\mathbf{B}_{t+1}  & \leftarrow   \mathbf{B}_t \mathbf{h} \Big( \beta \mathbf{C}_{\text{up}} \odot \kappa_{\text{up}}\big( 2 \mathbf{B}_t^{-1} \mathbf{g}_{\Sigma_t} \mathbf{B}_t^{-T} \big) \Big)
 \end{aligned}
$$` 
* obtain an update to solve `$\eqref{5}$` with a group-structural invariance:
`$$
\begin{aligned}
...
<fieldset class="field-set" markdown="1">
If `$\mathbf{B} \in {\cal{B}_{\text{up}}}(k)$`,  the update in `$\eqref{7}$` is invariant under any (group) transform `$\mathbf{R}^T \in  {\cal{B}_{\text{up}}}(k)$` of  `$\eqref{5}$`  such as `$\min_{y \in \mathcal{R}^p}  \ell(\mathbf{R} \, \mathbf{y})$`.
</fieldset>
</details>
...
Our method gives adaptive-gradient updates with a group-structural invariance by
...
Our NGD 
* is applied to structured mixture: `$q(\mathbf{w}|\tau)=\frac{1}{K}\sum_{k=1}^{K}q(\mathbf{w}|\mu_k,\mathbf{S}_k)$`
