...
`$\phi_{\lambda_t}$` is
<span style="color:red"> tight </span> at `$\lambda_t$`: $\lambda_t \equiv \phi_{\lambda_t} (\eta_0)$, and  we assume `$\eta_0 =\mathbf{0}$` the relative origin.
`$$ 
\begin{aligned} 
...
Update on `$\mathbf{S}_{t+1}=\mathbf{B}_{t+1}\mathbf{B}_{t+1}^T$` is like update of `$\eqref{2}$`, where `$\mathbf{G}_t := E_{q(w|\tau_t)}{ \big[ \nabla_w^2 \ell(\mathbf{w}) ] } -\gamma \mathbf{S}_t$`.
`$$
\begin{aligned}
&\mathbf{S}_{t+1} =   \underbrace{ \overbrace{(1-\beta \gamma)\mathbf{S}_t + \beta  E_{q(w|\tau_t)}{ \big[ \nabla_w^2 \ell(\mathbf{w}) \big] }}^{\text{standard NGD on $\mathbf{S}$ }} + \color{red}{ \frac{\beta^2}{2} \mathbf{G}_t \mathbf{S}_t^{-1}\mathbf{G}_t}
 ...
 \end{aligned}
$$` where $\mathbf{B}$ is a <span style="color:red">**dense**</span> matrix in matrix group `$\mathrm{GL}^{p\times p}$`.


------
# Gaussian example with structured covariance


 Dense     | Diagonal       | lower-triangular | upper-triangular | Hierarchical | Kronecker product
:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:
<img src="/img/icml2021-group-full.png"  width="250"/> | <img src="/img/icml2021-group-diag.png"  width="250"/> |  <img src="/img/icml2021-group-low.png"  width="250"/> |  <img src="/img/icml2021-group-up.png"  width="250"/> |  <img src="/img/icml2021-group-hie.png"  width="250"/> |  <img src="/img/icml2021-group-kro.png"  width="250"/> 
