...
$$`
 ...
 Our NGD in `$  \lambda $` is shown below, where we assume $\eta_0=\mathbf{0}$.
`$$
\begin{aligned}
\lambda_{t+1} = 
...
`$\mathbf{g}_\mu$` and `$\mathbf{g}_{\Sigma}$` are Euclidean gradients of `$\eqref{1}$` computed via Stein's lemma:
`$$
\begin{aligned}
