...
$$`
where `$w$` is the decision variable,  `$\ell(\mathbf{w})$` is a loss function, `$\Omega_\tau$` is the parameter space of `$q$`, and `$\gamma\ge 0$` is a constant.
...
Our NGD performs updates in local parameter spaces while maintaining structures via matrix groups in auxiliary parameters. This decoupling enables a tractable NGD that exploits the structures in auxiliary parameter spaces.
...
\begin{aligned}
\lambda_{t+1} = 
  \Big\{ \begin{array}{c} \mu_{t+1} \\ \mathbf{B}_{t+1} \end{array} \Big\} =
...
Note that `$\mathbf{g}_\mu$` and `$\mathbf{g}_{\Sigma}$` are Euclidean gradients of `$\eqref{1}$` computed via Stein's lemma:
`$$
\begin{aligned}
...
# Gaussian Example with Flexiable Structured Covariance
...
A structured Gaussian example: Auxiliary  parameter $\mathbf{B}$ lives in  a structured space (matrix Lie group):  `${\cal{B}_{\text{up}}}(k)$`,
...
Maps `$\psi$`, `$\phi_{\lambda_t}$` are defined in the previous section. `$\kappa_{\text{up}}(\mathbf{X})$` extracts non-zero entries of `${\cal{M}_{\text{up}}}(k)$` from `$\mathbf{X}$`.
...
\mathbf{B}_t  \mathbf{h} \Big( \beta \mathbf{C}_{\text{up}} \odot \kappa_{\text{up}}\big( 2 \mathbf{B}_t^{-1} \mathbf{g}_{\Sigma_t} \mathbf{B}_t^{-T} \big) \Big)  \end{array} \Big\}   
 \end{aligned}
$$`
where `$\odot$` is the elementwise product , `$\kappa_{\text{up}}(\mathbf{X}) \in {\cal{M}_{\text{up}}}(k)$`, and
`$ \mathbf{C}_{\text{up}} = 
 \begin{bmatrix}
...
Note that `$ \mathbf{B}_{t+1} \in {\cal{S}_{\text{up}}}(k)$` thanks to the property of `$\cal{M}_{\text{up}}(k)$`:
...
we formulate a new problem over Gaussian `$q(\mathbf{w}|\tau)$` with structured covariance.
`$$
\begin{aligned}
   \min_{\tau \in \Omega_\tau} E_{q(w|\tau)} \big[ \ell(\mathbf{w}) \big] +  E_{q(w|\tau)} \big[ \log q(\mathbf{w}|\tau)\big],
 \end{aligned}\tag{5}\label{5}
$$` where $\tau=(\mu,\mathbf{S})$ with mean $\mu$ and precision matrix $\mathbf{S}=\Sigma^{-1}$.

Use our NGD to solve `$\eqref{5}$` with $\mathbf{S}=\mathbf{B}\mathbf{B}^T$, which gives the following update
`$$
\begin{aligned}
...
Obtain an update to solve `$\eqref{4}$` with a group-structural invariance:
`$$
\begin{aligned}
...
$$`
by using `$\eqref{3}$` evaluated at the mean `$\mu_t$`
`$$
\begin{aligned}
...
<fieldset class="field-set" markdown="1">
<legend class="leg-title">Group-structural invariance:</legend>
If `$\mathbf{B} \in {\cal{B}_{\text{up}}}(k)$`,  the update in `$\eqref{6}$` is invariant under any (group) transform `$\mathbf{R}^T \in  {\cal{B}_{\text{up}}}(k)$` of  `$\eqref{4}$`  such as `$\min_{y \in \mathcal{R}^p}  \ell(\mathbf{R} \, \mathbf{y})$`.
</fieldset>



<fieldset class="field-set" markdown="1">
<legend class="leg-title">Time complexity:</legend>
...
## Structured Adaptive-gradient Methods for DL
...
Obtain adaptive-gradient updates with a structural invariance by
 approximating `$\nabla_{\mu_t}^2 \ell( \mu)$` using the Gauss-Newton in `$\eqref{7}$`.
...
Easy to parallelize by Auto-Diff, No sequential conjugate-grad (CG) steps
</fieldset>
...
##  Variational Inference (VI) with Gaussian Mixtures

Our NGD can be easily exteneded to structured mixture: `$q(\mathbf{w}|\tau)=\frac{1}{K}\sum_{k=1}^{K}q(\mathbf{w}|\mu_k,\mathbf{S}_k)$`
      
Obtain natural-gradient VI for structured Gaussian mixtures with MC gradients
      
Example: approximate a 80-dim mixture of Student's t distributions
