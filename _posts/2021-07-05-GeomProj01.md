...
<img src="/img/icml2021-fig03.png"  width="500"/> |  When `$\tau$` space has a local vector-space structure, <br> standard NGD in $\tau$ space is a special case of our NGD,  <br> where we choose `$\psi$` to be the identity map and `$\phi_{\lambda_t}$` to be a linear map.
