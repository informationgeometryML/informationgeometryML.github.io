...
<img src="/img/icml2021-fig03_new.png"  width="500"/> |  When `$\tau$` space has a local vector-space structure, <br> standard NGD in $\tau$ space is a special case of our NGD,  <br> where we choose `$\psi$` to be the identity map and `$\phi_{\lambda_t}$` to be a linear map.
...
>       `$$
>           \begin{aligned}
>                &\mathbf{h}\big(\mathbf{M}\big) \in {\cal{B}_{\text{up}}}(k) \text{ for }  \mathbf{M} \in \text{sub-space of Lie algebra } \cal{M}_{\text{up}}(k) \,\,\,\,  &(\text{by design, } \mathbf{h}(\cdot) \text{ is a Lie-group retraction}) \\
