...
In {% cite glasmachers2010exponential %}, the matrix exponential map is used for tractable natural-gradients.
However, the exponential map is not numerical stable and has high iteration cost.

To fix these issues, 
we propose using map `$\mathbf{h}()$` while keeping natural-gradient computation tractable.
</div>
