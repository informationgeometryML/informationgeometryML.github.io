...
Motivations:
* use low-rank or sparse covariance/precision matrix for Gaussian models
* have flexiable posterior approximation for Bayesian models
* use structured second-order information for optimization
* enable non-diagonal adaptive gradient methods for deep learning
* have a systemic approach to incorporate a range of (matrix) structures
...
Issues in existing works:
* Limited structures and singularity of the Fisher information matrix
* Ad-hoc approximations for cost reductions 
* Complicated and inefficient NG computation 


Existing approach for Gaussian rank-one covariance   | Our NGD for rank-one covariance 
:-------------------------:|:-------------------------:
...
= \underbrace{  \color{green}{\Big(\begin{array}{cc} \mathbf{I}_p & 0 \\ 0 & 2 \mathbf{I}_{p^2} \end{array} \Big)^{-1}}  }_{ \text{inverse of exact FIM } } \Big[\begin{array}{c}  \mathbf{B}_t^{-1} \mathbf{g}_{\mu_t} \\ \mathrm{vec}( -2\mathbf{B}_t^{-1} \mathbf{g}_{\Sigma_t} \mathbf{B}_t^{-T}) \end{array} \Big] && \text{( tractable NG: easy to inverse the FIM ) }
\end{aligned}
$$`
