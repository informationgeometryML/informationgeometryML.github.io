...
Our NGD 
* can use structured Gaussian mixtures as flexiable variational distributions: `$q(\mathbf{w}|\tau)=\frac{1}{C}\sum_{c=1}^{C}q(\mathbf{w}|\mu_c,\mathbf{S}_c)$`
...
|:--:|:--|
|:  <font size="4"> Comparison between our approximation (`$k=5$`) and the ground-truth </font> :||
