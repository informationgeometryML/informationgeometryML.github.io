...
\begin{aligned}
 ...
 \end{aligned}
$$`
where tractable natural-gradient  `$\hat{\mathbf{g}}_{\eta_0}^{(t)}$`  at `$\eta_0=\{\delta_0, \mathbf{M}_0\}$` tied to `$\lambda_t=\{\mu_t,\mathbf{B}_t\}$`:
...
The second-order term shown in red is used for the positive-definite constraint {% cite  lin2020handling %} known as a retraction in Riemannian gradient descent (RGD).  A higher-order term `$O(\beta^3)$` will be used for structured precision in the next section. 
