...
second-order Taylor  approximation of any f-divergence `$\mathrm{D}_f(\mathbf{y},\eta_k)$` at `$y=\eta_k$` such as the KL divergence `$\mathrm{D}_f(\mathbf{y},\eta_k)= \mathrm{KL} [p(\mathbf{w}|\eta_k) || p(\mathbf{w}|\mathbf{y})]$`.
...
We have the following additional results, when the f-divergence is choosen to be a KL divergence.
...
This establishes the connection between NGD and proximal gradient descent.
...
The
second-order Taylor approximation  of the reverse KL divergence at `$\mathbf{y}=\eta_k$` gives the same approximation as `$\frac{1}{2} (\mathbf{y}-\eta_k)^T \mathbf{F}_{\eta}(\eta_k) (\mathbf{y}-\eta_k)$`.
This establishes the connection between NGD and mirror descent.
...
Without the Taylor approximation,  `$\eqref{2}$` and  `$\eqref{4}$` are distinct updates even when the KL divergence is used.
</div>
