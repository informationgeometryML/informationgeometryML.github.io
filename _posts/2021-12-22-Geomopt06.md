...
* In constrained cases, either  `$\Omega_m$` or `$\Omega_\eta$`  can be an arbitrary open subset in `$\mathcal{R}^K$`.
</div>
...
This approach is closedly related to proximial-gradient descent.
Recall that in
...
# Efficient Riemannian Gradient Descent for Multivariate Gaussians

## Natural Gradient Descent as a Newton's update


## Riemannian Retraction Map
