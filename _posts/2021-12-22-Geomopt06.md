...
When the f-divergence is choosen to be the KL divergence `$\mathrm{D}_f(\mathbf{y},\eta_k)= \mathrm{KL} [p(\mathbf{w}|\eta_k) || p(\mathbf{w}|\mathbf{y})]$`, we have more  nice results.
