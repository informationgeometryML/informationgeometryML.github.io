...
second-order Taylor expansion  at 
...
The
second-order Taylor expansion of the reverse KL divergence at `$\mathbf{y}=\eta_k$` gives the same approximation as `$\frac{1}{2} (\mathbf{y}-\eta_k)^T \mathbf{F}_{\eta}(\eta_k) (\mathbf{y}-\eta_k)$`.
