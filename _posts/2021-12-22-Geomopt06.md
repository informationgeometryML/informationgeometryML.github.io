...
the FIM `$\mathbf{F}_\eta(\eta_k)$` evaluted at `$\eta_k$` as a metric for the projection, where we use the [weighted inner product]({{ site.baseurl }}{% post_url 2021-10-04-Geomopt02 %}
#distance-induced-by-the-fisher-rao-metric) to measure the distance.
