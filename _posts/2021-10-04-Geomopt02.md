...
The distance (and orthogonality) between two <span style="color:red">vectors</span> at  <span style="color:red">point `$\tau_0$`</span>  point is also induced by FIM since we can define them by the inner product as
`$$
\begin{aligned}
...
`$\tau_1$`, which involves the concept of parallel transport in a curved space. For simplicity, we do not how to parallelly transport a
vector in this post.
</div>
