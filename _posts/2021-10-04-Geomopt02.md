...
Each possible vecotor `$\mathbf{v}$` lives in the same (vector) space at current point `$\mathbf{\tau}_0$`.
</div>
...
\end{aligned}
$$`
where vector `$\mathbf{v}$` and `$\mathbf{w}$` live in the (vector) space at point `$\tau_0$`.
...
* vector `$\mathbf{v}$` lives in a distinct space. This space is called the tangent/gradient vector  space `$\mathcal{R}^k$` at current point `$\tau_0$`
