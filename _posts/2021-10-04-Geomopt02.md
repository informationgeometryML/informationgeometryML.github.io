...
Given a scalar function `$f(\mathbf{\tau})$` with an intrinsic parameter `$\tau$`, we define its (un-normalized) **Riemannian** (steepest) gradient as `$ \mathbf{F}^{-1}(\mathbf{\tau}) \nabla_\tau f(\mathbf{\tau})$` if its (un-normalized) **Euclidean** (steepest) gradient is `$\nabla_\tau f(\mathbf{\tau})$`.
