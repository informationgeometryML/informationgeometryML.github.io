...
* vector `$\mathbf{v}$` stays in a distinct space. This space is called the tangent vector space `$T_{\tau_0} (\Omega_\tau)$` at current point `$\tau_0$`
</div>
