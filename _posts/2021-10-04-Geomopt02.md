...
Note that we only define the distance of vectors in the green space. In the $\Omega_\tau$ space shown in red, we do not define the length between `$\tau_0$` and `$\tau_0 +t\mathbf{v}$`.
