...
Given a scalar function $f(\mathbf{\tau})$, if its **Euclidean** gradient is $\nabla_\tau f(\mathbf{\tau})$, its **Riemannian** gradient is $ \mathbf{F}^{-1}(\mathbf{\tau}) \nabla_\tau f(\mathbf{\tau})$.
...
We can define vector additions and real scalar products by tangent directions of curves in the manifold. 
