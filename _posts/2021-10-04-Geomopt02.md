...
Therefore, we can similarly define a directional derivative at `$\mathbf{\tau}_0$` along Riemannian vector[^1] $\mathbf{v}$ as `$\lim_{t \to 0} \frac{f(\mathbf{\tau}_0+t\mathbf{v}) - f(\mathbf{\tau}_0) }{t}$`, where $t$ is a scalar real number. The main point is that `$\mathbf{\tau}_0+t\mathbf{v}$` stays in the parameter space `$\Omega_\tau$` thanks to the **local vector space** structure.
...
Under **intrinsic** parameterization $\mathbf{\tau}$, the directional derivative remains the same[^2] as in the Euclidean case thanks to the **local vector space** structure in `$\Omega_\tau$`.
...
## Footnotes:
[^1]: A Riemannian gradient is a coordinate representation of a [contravariant vector](https://en.wikipedia.org/wiki/Covariance_and_contravariance_of_vectors) (A.K.A. a Riemannian vector).

[^2]: For simplicity, we avoid defining a (coordinate-free) [covariant derivative](https://en.wikipedia.org/wiki/Covariant_derivative). Given  a scalar field/function on a manifold, the standard (Euclidean) coordinate derivative is a coordinate representation of a [covariant vector](https://en.wikipedia.org/wiki/Covariance_and_contravariance_of_vectors) (A.K.A. a Riemannian covector) induced by the covariant derivative.
