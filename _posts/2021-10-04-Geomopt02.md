...
>
>In the literature, a natural gradient w.r.t. `$\mathbf{S}$` is  defined as `$\mathrm{MatH}(\mathbf{v})$`, where `$\mathbf{v}$` is a valid natural gradient w.r.t. intrinsic parameter `$\mathrm{vech}(\mathbf{S})$`. 
</fieldset>
</details>
</div>
