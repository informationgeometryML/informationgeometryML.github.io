...
\end{aligned},
$$`
where `${\mathbf{g}}_\lambda$` can be computed via the vector-Jacobian product in any standard auto-diff toolbox given that `$\hat{\mathbf{g}}_\tau$` is pre-computed.
...
Thus, the optimal solution of Eq. `$\eqref{2}$` under parametrization $\tau$ and $\lambda$ are equivalent since both the length and the directional derivatives remains the same.
