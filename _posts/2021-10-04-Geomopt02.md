...
Under **intrinsic** parametrization $\tau$, we have `$\Omega_\tau \subset \mathcal{R}^2$`. Thus, We can perform the operation in $\Omega_\tau$ space: `$\tau_0 +t\mathbf{v}(\tau_0)$` when step-size $t$ is small enough.
