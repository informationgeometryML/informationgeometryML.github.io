...
------
Given a smooth scalar funcion $\min_{\tau \in \mathcal{R}^n } f(\mathbf{\tau})$ in a flat (Euclidean) space, we can define the (Euclidean) steepest direction at current `$\mathbf{\tau}_0$` as the solution to the following optimization problem in terms of a directional derivative w.r.t. vector $\mathbf{v}$, where we assume `$\nabla_\tau f(\mathbf{\tau}_0)  \neq \mathbf{0}$`.
`$$
\begin{aligned}
...
Recall that a manifold should be locally like a flat (Euclidean) space under [**intrinsic** parameterization]({{ site.baseurl }}{% post_url 2021-09-06-Geomopt01 %}#intrinsic-parameterizations) `$\mathbf{\tau}$`.
...
`$$
\begin{aligned}
\big( \mathbf{v}^{(opt)}_{\tau}  )^T \nabla  f_\tau(\mathbf{\tau}_0) &=  \big( \mathbf{v}^{(opt)}_{\lambda}  )^T \nabla  f_\lambda(\mathbf{\lambda}_0) \\
