...
Under **intrinsic** parameterization $\mathbf{\tau}$, the directional derivative remains the same[^1] as in the Euclidean case thanks to the **local vector space** structure in `$\Omega_\tau$`.
...
## Footnotes:
[^1]: For simplicity, we avoid defining a (coordinate-free) [covariant derivative](https://en.wikipedia.org/wiki/Covariant_derivative). For a scalar field/function on a manifold, the standard (Euclidean) coordinate derivative is a parametric representation of a [covariant vector](https://en.wikipedia.org/wiki/Covariance_and_contravariance_of_vectors) (A.K.A. a covector) induced by the covariant derivative.
