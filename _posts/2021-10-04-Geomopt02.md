...
**Warning**: We define the distance of vectors in the green space which a (global) vector space at `$\tau_0$`. Any Riemannian gradient (e.g., `$\mathbf{v}(\tau_0)$`) evaluated at `$\tau_0$` lives in this space.
We do not define the distance between `$\tau_0$` and `$\tau_0 +t\mathbf{v}(\tau_0)$` in the $\Omega_\tau$ space. It is locally like a vector space, which is open and bounded.
Parametric representations of points in the top half of (curved) sphere live in the $\Omega_\tau$ space.
In intrinsic parametrization $\tau$, we are allowed to perform a vector addition in the $\Omega_\tau$ space such as `$\tau_0 +t\mathbf{v}$` if step-size $t$ is small enough.
...
>and the blue line segment $\bar{\gamma}(t)= (t v_{x} , t v_y  ) \in \Omega_\tau $, where $t$ must small enough. 
>
