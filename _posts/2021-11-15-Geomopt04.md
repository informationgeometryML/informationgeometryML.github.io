...
To avoid writing down the differential equations of a geodesic[^1] (i.e., [Christoffel symbols](https://en.wikipedia.org/wiki/Levi-Civita_connection#Christoffel_symbols) or [connection 1-form in a section](https://en.wikipedia.org/wiki/Connection_form#Example:_the_Levi-Civita_connection)), 
...
The invariance of this update is due to the uniqueness of the geodesic and the invariance of [the Euler-Lagrange equation](https://en.wikipedia.org/wiki/Solving_the_geodesic_equations). We will not discuss this further in this post to avoid complicated derivations. 
...
By using this Taylor approximation,
we essentially use the distance defined in a (Riemannian) gradient space to approximate the distance in a parameter space.
</div>
