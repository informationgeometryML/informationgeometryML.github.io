...
We will discuss in
[Part V]({{ site.baseurl }}{% post_url 2021-12-14-Geomopt05 %}#minimal-parametrizations-of-exponential-family), for a special parametrization `$\eta$` (known as a natural parametrization) of [exponential family]({{ site.baseurl }}{% post_url 2021-12-14-Geomopt05 %}#exponential-family), the FIM under this parametrization can be computed  as
`$\mathbf{F}_\eta(\eta) = \nabla_\eta^2 A_\eta(\eta)$`, where `$A_\eta(\eta)$` is a strictly convex function.
