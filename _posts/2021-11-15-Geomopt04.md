...
We could also use other "distances" denoted by `$\mathrm{D}(.,.)$` (i.e., Kullback–Leibler divergence or other f-divergence) to define the length between two points in a parameter space.
