...
Contrastingly,  the Fisher information matrix is a valid manifold metric. Recall that we can compute the Fisher matrix  as `$\mathbf{F}_\tau(\tau) = E_{p(w|\tau)}\big[ -\nabla_\tau^2 \log p(w|\tau) \big]$`. Given a non-linear intrinstic reparametrization `$\lambda$`, the Jacobian matirx `$\mathbf{J}(\tau_k)$` is no longer a constant matrix.
the [transformation rule]({{ site.baseurl }}{% post_url 2021-11-02-Geomopt03 %}#transformation-rule-for-the-fisher-information-matrix) still holds thanks to the expectation of the score.
