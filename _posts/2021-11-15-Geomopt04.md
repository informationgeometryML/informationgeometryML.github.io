...
When `$\mathrm{D}(y,\tau_k)$` is a secord-order Taylor approximation of  the KL divergence `$\mathrm{KL} [p(w|\tau_k) || p(w|y)]$` at `$y=\tau_k$`, this  proximal-gradient method recovers natural-gradient descent.
