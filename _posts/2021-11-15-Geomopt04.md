...
On the other hand,  the Fisher information matrix is a valid manifold metric. Recall that the Fisher matrix can be computed as `$\mathbf{F}_\tau(\tau) = E_{p(w|\tau)}\big[ -\nabla_\tau^2 \log p(w|\tau) \big]$`. The expectation plays a key role to guarantee that the metric follows the [transformation rule]({{ site.baseurl }}{% post_url 2021-11-02-Geomopt03 %}#transformation-rule-for-the-fisher-information-matrix) in non-linear cases as shown below.
