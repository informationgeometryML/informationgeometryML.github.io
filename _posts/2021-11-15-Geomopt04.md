...
Claim: when `$\mathrm{D}(\mathbf{y},\tau_k)$` is a secord-order Taylor approximation of a f-divergence `$\mathrm{D}_f(\mathbf{y},\tau_k)$` at `$\mathbf{y}=\tau_k$`, the proximal-gradient method recovers natural-gradient descent.
...
To see that, we will show that the second-order Talor approximation at $\mathbf{y}=\tau_k$ is
`$$
\begin{aligned}
\mathrm{D}(\mathbf{y},\tau_k) := \mathrm{D}_f(\tau_k,\tau_k) + (\mathbf{y}-\tau_k)^T  [\nabla_y  \mathrm{D}_f(\mathbf{y},\tau_k)\big|_{y=\tau_k}] + \frac{1}{2}(\mathbf{y}-\tau_k)^T [\nabla_y^2  \mathrm{D}_f(\mathbf{y},\tau_k)\big|_{y=\tau_k}] (\mathbf{y}-\tau_k) = \frac{1}{2} (\mathbf{y}-\tau_k)^T \mathbf{F}_\tau(\tau_k) (\mathbf{y}-\tau_k)
\end{aligned}
$$`


Therefore, the unconstrained proximal-gradient descent can be re-expressed as
`$$
\begin{aligned}
...
It is easy to see that we can obtain the natural-gradient update from the above expression.

Now, we show that the second-order Talor approximation at $\mathbf{y}=\tau_k$ can be re-expressed as below.
`$$
\begin{aligned}
...
the parameter space `$\Omega_\tau$` is constrained. In constrained cases, these methods could be distinct from each other. We will cover more about this point in [Part VI]({{ site.baseurl }}{% post_url 2021-12-22-Geomopt06 %}).
</div>
