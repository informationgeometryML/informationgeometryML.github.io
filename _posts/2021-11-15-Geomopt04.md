...
To avoid writing down the differential equations of a geodesic (i.e., [Christoffel symbols](https://en.wikipedia.org/wiki/Levi-Civita_connection#Christoffel_symbols) or [connection 1-form](https://en.wikipedia.org/wiki/Connection_form#Example:_the_Levi-Civita_connection)  ), 
...
In this section, we will make an additional but key assumption:
...
`$$
\begin{aligned}
\mathrm{D}_f(\mathbf{y},\tau_k) \geq  f( \int_w \frac{p(w|\mathbf{y})}{p(w|\tau_k)} p(w|\tau_k) dw )  = f(\int_w p(w|\mathbf{y})dw) =f(1) =0 .
\end{aligned}
$$`
