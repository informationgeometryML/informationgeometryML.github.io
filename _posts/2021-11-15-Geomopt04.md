...
The geodeisc is a solution of a system of ordinary differential equations (ODE) with the following initial conditions.
`$$
\begin{aligned}
\gamma(0) = x_0; \,\,\,\,\,\,
...
\frac{d \gamma_\tau(t) }{d t} \Big|_{t=0} = \mathbf{v}_{\tau_0}
\end{aligned}
$$` where the completeness is required if we want the domain of the expoential map to be the whole tangent space.
...
Natural-gradient descent can be viewed as a first-order (linear) approximation of the geodesic, which implies that natural-gradient descent is indeed inexact Riemannian gradient descent.
...
We can similarly define an approximated  expoential map (A.K.A. the retraction map) as
`$$
\begin{aligned}
...
Therefore, inexact Riemannian gradient descent is defined as 
`$$
\begin{aligned}
...
to do: mention the mirror descent and `$\mathrm{D}(y,\tau_k)$` becomes exact for expoential family (will be discussed more in Part V)
