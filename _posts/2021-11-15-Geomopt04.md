...
In the following section, we assume `$y$` and `$\tau_k$` are two members in
the same parametric distribution family `$p(w|\tau)$`.
The f-divergence can be defined in a more general setting.
...
a f-divergence is always non-negative as  `$\mathrm{D}_f(y,\tau) \geq  f( \int_w \frac{p(w|y)}{p(w|\tau)} p(w|\tau) dw )  = f(\int_w p(w|y)dw) =f(1) =0 $`.

## Proximal-gradient descent
Given such a "distance", we can perform unconstrained proximal-gradient descent as shown below.
`$$
\begin{aligned}
...
Claim: when `$\mathrm{D}(y,\tau_k)$` is a secord-order Taylor approximation of a f-divergence `$\mathrm{D}_f(y,\tau_k)$` at `$y=\tau_k$`, the proximal-gradient method recovers natural-gradient descent.
...
To see that, we first show that the Hessian of any f-divergence `$\mathrm{D}_f(y,\tau_k)$` at `$y=\tau_k$` becomes the Fisher information matrix `$\mathbf{F}_\tau(\tau_k)$`.
`$$
\begin{aligned}
...
&= \int_w [\ddot{f}\big(\frac{p(w|y)}{p(w|\tau_k)}\big) \frac{\nabla_y p(w|y)}{p(w|\tau_k)}\frac{\nabla_y^T p(w|y)}{p(w|\tau_k)} +\dot{f}\big(\frac{p(w|y)}{p(w|\tau_k)}\big) \frac{\nabla_y^2 p(w|y)}{p(w|\tau_k)} ] p(w|\tau_k) dw \Big|_{y=\tau_k} \\
...
Thus the second-order Talor approximation at $y=\tau_k$ is
`$$
\mathrm{D}(y,\tau_k) := \mathrm{D}_f(\tau,\tau_k) + 0 + (\mathbf{y}-\tau_k)^T \mathbf{F}_\tau(\tau_k)
$$`
...
`$$
\begin{aligned}
\tau_{k+1} \leftarrow \arg\min_{y \in  \mathcal{R}^K  } \{ \langle \mathbf{g}_{\tau_k}, \mathbf{y} \rangle   + \frac{1}{\alpha} (\mathbf{y}-\tau_k)^T \mathbf{F}_\tau(\tau_k) (\mathbf{y}-\tau_k) \}
\end{aligned}
$$` where `$\mathbf{g}_{\tau_k}$` is a Eulcidean gradient and the parameter space is unconstrained. 
...
In [Part V]({{ site.baseurl }}{% post_url 2021-12-14-Geomopt05%}#natural-gradient-descent-as-unconstrained-mirror-descent)   , we will show that  `$\mathrm{D}(y,\tau_k)$` can also be an exact KL divergence for exponential family.
