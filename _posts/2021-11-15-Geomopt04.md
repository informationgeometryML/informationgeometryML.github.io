...
>
> As we will shown in [Part V]({{ site.baseurl }}{% post_url 2021-12-14-Geomopt05 %}), the iteration cost of natural-gradient descent in this parameter space $\Omega_\lambda$ will be $O(d^3)$ if we exploit structures of the exact non-singular FIM.
...
On the other hand,  the Fisher information matrix is a valid manifold metric. Recall that the Fisher matrix can be computed as `$\mathbf{F}_\tau(\tau) = E_{p(w|\tau)}\big[ -\nabla_\tau^2 \log p(w|\tau) \big]$`. The expectation plays a key role to ensure the metric follows the [transformation rule]({{ site.baseurl }}{% post_url 2021-11-02-Geomopt03 %}#transformation-rule-for-the-fisher-information-matrix) in non-linear cases as shown below.
...
\mathbf{F}_\tau(\tau_k) &= E_{p(w|\tau_k)}\big[ -\nabla_\tau^2 \log p(w|\tau_k) \big]  \\
 ...
 &= E_{p(w|\lambda_k)}\big[ -\nabla_\tau [ \mathbf{J}^T \nabla_\lambda \log p(w|\lambda_k)]  \big]  \\
  &= -    \mathbf{J}^T   E_{p(w|\lambda_k)}\big[  \nabla_\tau  \nabla_\lambda \log p(w|\lambda_k) \big]   - [\nabla_\tau ( \mathbf{J}^T )] \underbrace{ E_{p(w|\lambda_k)}\big[  \nabla_\lambda \log p(w|\lambda_k) \big] }_{=0}  \\
&= -    \mathbf{J}^T   E_{p(w|\lambda_k)}\big[  [\nabla_\lambda^2 \log p(w|\lambda_k) ] \mathbf{J} \big] \\
&= -    \mathbf{J}^T   E_{p(w|\lambda_k)}\big[  \nabla_\lambda^2 \log p(w|\lambda_k) \big] \mathbf{J}\\
&=\mathbf{J}^T\mathbf{F}_\lambda(\lambda_k) \mathbf{J} 
\end{aligned}
$$`
