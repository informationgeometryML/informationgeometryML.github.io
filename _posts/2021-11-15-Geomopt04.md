...
Recall that in Part III we only define a distance btween two Riemannian gradients evaluated at the same point. We can use the length of a geodesic to define the distance between two points on the manifold.
...
\frac{d \gamma_\tau(t) }{d t} \Big|_{t=0} = \mathbf{v}_{\tau_0}
\end{aligned}
$$` Technically, we should require the manifold to be geodesically complete so that the domain of the expoential map is the whole tangent space.
...
\end{aligned}
$$` Note that this approximation does not guarantee that the approximated geodesic stays on the manifold for any $t$.
