...
`$ \{ \partial_{\tau_i} \log p(w|\tau) \} $`  should be linearly independent. In other words, `$\sum_i c_i \partial_{\tau_i} \log p(w|\tau)= 0 $`, where constant `$c_i$` must be zero and the value of `$c_i$` does not depent on  `$w$`.
...
>
>We will show that the regularity condition holds. Consider a 1-dimensional Gaussian family `$ \{ \mathcal{N}(w |\mu,\sigma) \Big| \mu \in \mathcal{R}, \sigma>0 \}$` with mean `$\mu$`, variant `$\sigma$`, and parametrization `$\tau = (\mu,\sigma) $`.
...
>Thus, by Eq. `$\eqref{4}$`, FIM under this  parameterization is
>
...
------
We can define the dimension of a manifold by using the dimension of an intrinsic parametrization.
Mathematically speaking, we can show that any intrinsic parametrization of a manifold has the same degrees of freedom.
This also gives us a tool to help identify non-manifold examples.
We now illustrate this by examples.

A unit circle is a 1-dimensional manifold.  An open unit ball is a 2-dimensional manifold.
However, a closed
unit ball is NOT a manifold since its interior is a unit circle and its boundary is an open unit ball.
The  circle and the  unit ball do not have the same dimensionality.
...
We will disucss more about the following statistical  manifolds in [Part II]({{ site.baseurl }}{% post_url 2021-10-04-Geomopt02 %}#riemannian-steepest-direction).


1-dim Gaussian with zero mean  | $d$-dimensional Gaussian with zero mean
:-------------------------:|:-------------------------:
 `$ \{ \mathcal{N}(w |0,s^{-1}) \Big|  s>0 \}$` with precision `$s$` <br /> under intrinsic parameterization `$\tau = s $` | `$ \{ \mathcal{N}(\mathbf{w} |\mathbf{0},\mathbf{S}^{-1}) \Big| \mathrm{MatH}(\tau) = \mathbf{S}   \succ \mathbf{0} \}$` with precision `$\mathbf{S}$`  <br /> under intrinsic parameterization `$\tau = \mathrm{vech}(\mathbf{S})$` is  a $\frac{d(d+1)}{2}$-dim array.  <br />  Map $\mathrm{MatH}()$ is the inverse map of <br /> the [half-vectorization function](https://en.wikipedia.org/wiki/Vectorization_(mathematics)#Half-vectorization) `$\mathrm{vech}()$`. <br />   `$\mathrm{vech}(\mathbf{S})$` is a `$\frac{d(d + 1)}{2}$`-dim array <br />  obtained by vectorizing only the lower triangular part of (symmetric) `$\mathbf{S}$`.
