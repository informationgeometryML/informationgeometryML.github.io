...
when we specify these two parametrizations. The uniqueness implies the FIM is non-singular at a point while the smoothness (and the openness) implies that the
...
In other words, an intrinsic parameterization is still necessary, just that the openness requirement is not explicit and hidden within the regularity condition.
