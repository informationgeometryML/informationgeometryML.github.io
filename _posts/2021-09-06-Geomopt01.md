...
An arbitrary parameterization may not always be appropriate for a smooth manifold {% cite tu2011introduction  %}. The requirement is that the parameterization should be such that the manifold is locally a *flat* vector space, for example, how the curved Earth surface looks flat to us, locally.
...
We refer to the parametrizations that satisfy this condition as an intrinsic parameterization [^1], which as we will see will lead to non-singular FIMs of `$K$`-by-`$K$`[^1].
Non-singularity is crucial for algorithms such as natural-gradient descent, as we will see in [Part IV]({{ site.baseurl }}{% post_url 2021-11-15-Geomopt04 %}#natural-gradient-descent-in-an-intrinsic-parameter-space).
...
>Parameterization 2 (a non-intrinsic parameterization):
>
