...
>
>
> 3. Well-defined paramtrization with **non-intrinsic**  $2$-by-$2$ (potentially asymmetric) parameter matrix `$\Sigma$`: `$\Big \{ \exp\Big( -\frac{1}{2}\Big[  w^T \big(\frac{ \Sigma + \Sigma^{T} }{2}\big)^{-1} w + \log \mathrm{det}\big(\frac{ \Sigma + \Sigma^{T} }{2}\big) + 2 \log (2\pi)  \Big] \Big)  |  \frac{ \Sigma + \Sigma^{T} }{2} \succ \mathbf{0},  \Sigma \in \mathcal{R}^{2 \times 2} \Big\}$`, where we have to explicitly enforce the symmetric constraint.
