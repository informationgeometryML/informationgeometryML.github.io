...
* We require that the parameter space of $\tau$, denoted by $\Omega_\tau$, be an **open** set in `$\mathcal{R}^K$`, where `$K$` is the number of entries of a parameter array. Intuitively, this ensures a local vector-space structure throughout the parameter space, which then ensure that a small, local perturbation at all points in `$ E $` stay within $\Omega_\tau$.

* We also require that `$ E $`  **uniquely** and **smoothly** represents a sub-set of points in a manifold. The condition ensures arbitrary transformations of the parameterizations should still keep the local perturbations within the manifold. 
We require that there exists **bi-jective** maps among such parameterizations and that their inverse maps are also **smooth**. The second condition[^2] can be abstractly defined.
