...
Note that this regularity condition implicitly assumes that the parameter space `$\Omega_\tau$` is an open set in `$\mathcal{R}^K$` due to the definition of the partial derivatives, where K is the number of entries in parameter array `$\tau$`. So an intrinsic parameterization is still necessary, just that the requirement is not explicit and hidden within the condition.
