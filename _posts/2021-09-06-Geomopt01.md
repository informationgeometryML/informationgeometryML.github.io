...
Note that `$\log \mathcal{N}(\mathbf{w} |\mathbf{0},\mathbf{S}^{-1}) := -\frac{1}{2} (d \log (2\pi) + \log \mathrm{det}(  \mathbf{V} ) + \mathbf{w}^T \mathbf{V}^{-1} \mathbf{w} ) $`, where we explicitly enforce the symmetric constraint as `$\mathbf{V}:=\frac{\mathbf{S}^{-1} + \mathbf{S}^{-T}}{2}$`.
We  can use 
