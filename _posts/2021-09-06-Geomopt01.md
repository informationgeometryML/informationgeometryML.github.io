...
Goal
------
This blog post focuses on the Fisher-Rao metric, which gives rise to the Fisher information matrix (FIM). We will introduce the following concepts useful to ensure non-singular FIMs
* Regularity conditions and intrinsic parameterization of a distribution
...
# Motivation
------
The goal of this blog is introduce the geometric structures associated with probability distribution. Let's start with some motivation: why should we care about such geometric structures?

The answer is that by exploiting geometric structures, we can
...
This is true for a majority of machine learning methods, all of which make use of probability distributions of various kinds. 

Below, we give some common examples from the literature. A reader familiar with such examples can skip this part. 

> **Least Squares** (frequestist estimation): 
>
...
>$$`
>where `$ p(x,y | \tau) = \mathcal{N}(y | x^T\tau,1) p(x) $` is assumed to be the data-generating distribution. (`$ \mathcal{N} (y | m, v) $` denotes a normal distribution over `$ y $` with mean `$ m $` and variance `$ v $`).
>
> Well-known algorithms such as  [**Fisher scoring**](https://en.wikipedia.org/wiki/Scoring_algorithm#Fisher_scoring)  and **(emprical) natural-gradient descent** {% cite   martens2020new %} are commonly used methods that exploit the geometric structure of `$p(x,y | \tau)$`. These are examples of algorithms derived from a frequentist perspective, which can also be generalized to neural network {% cite   martens2020new %}.
...
> This approach is derived from a Bayesian  perspective, and can also be generalized to neural network cases {% cite lin2021tractable %} {% cite osawa2019practical %}.


> **Evolution Strategies** (Global optimization): 
>
...
| Example  &nbsp; &nbsp; &nbsp;    | `$w$`   &nbsp; &nbsp; &nbsp;     | `$ p(w|\tau) $`  |
|:------------|:-------------:| -----:|
| Least Square     | observation `$y$`  | `$p(y|x,\tau)$` |
...
In the rest of the post, we will mainly focus on the geometric structure of (finite-dimensional) parametric families, for example, a 1-dimensional Gaussian family.
...
------
We start by discussing a special type of parameterizations, we call intrinsic parameterizations of distributions which are useful to obtain non-singular FIMs.
An arbitrary parameterization may not always be appropriate for a smooth manifold {% cite tu2011introduction  %}. The requirement is that the parameterization should be such that the manifold is locally like a *flat* vector space, for example, how the curved Earth surface looks flat to us, locally.
...
We refer to the parametrizations that satisfy the following conditions as an intrinsic parameterization [^1], which as we will see will lead to non-singular FIMs of `$K$`-by-`$K$`[^1].
Both openness and non-singularity is crucial for algorithms such as natural-gradient descent, as we will see in [Part IV]({{ site.baseurl }}{% post_url 2021-11-15-Geomopt04 %}#natural-gradient-descent-in-an-intrinsic-parameter-space).

* Intuitively, with such a local vector-space structure, we ensure that a small, local perturbation at each point in `$ E $` stays within the parameter space of $\tau$ (denoted by $\Omega_\tau$). Technically, this requires $\Omega_\tau$ to be an **open** set in `$\mathcal{R}^K$`, where `$K$` is the number of entries of a parameter array.

* Moreover, we require that `$ E $`  **uniquely** and **smoothly** represents a sub-set of points in a manifold.
The second condition may be abstract since a manifold can be abstractly defined.
This condition becomes more clearly when we have two distinct intrinsic parameterizations such as $\tau$ and $\eta$ to represent the same sub-set in the manifold.
We require that there exists a **bi-jective** map from $\tau$ to $\eta$ such that the map and its
inverse map are both **smooth**. The smoothness of the inverse map is essential since these two parameterizations can be arbitrary. In other words, the condition should not depend on the order
when we speicify these two parametrizations. The uniqueness implies the FIM is non-singular at a point while the smoothness (and the openness) implies that the
FIM is non-singular in the whole parameter space.

We will now give a simple example of intrinsic parameterization on a unit circle in `$\mathcal{R}^2$` at the point `$ (0,1) $` highlighted in green.
...
Now, we discuss invalid cases, where not all conditions are satisified.

>Parameterization 2 (non-smoothness):
>
...
>
>This map $f$ is bijective and smooth. However, the parameter space is not open in $\mathcal{R}$, and its inverse map $f^{-1}$ is **not** continous at point $(0,1) \in  \mathcal{S}^1$. Therefore, this parametrization is not intrinsic.
> In fact, there does not exist a (single) **global** and intrinsic parametrization to represent the circle.

As shown in Parametrization 2, continuity (or smoothness[^2]) of the inverse map is essential when it comes to reparametrization (A.K.A. parameter transformation). Without such smoothness, the parameterization is not instrinsic, but smoothness, along with the inverse map, gives us a way to generate new intrinsic parameterizations. Essentially, in such case, the Jacobian matrix (to change between the parameterizations) is non-singular, and we can use the chain rule and inverse function theorem to jump between different intrinsic parameterizations.

>Parametrization 3 (non-openness):
>
...
>Parametrization 4 (non-uniqueness):
>
...
>
>This parametrization is not intrinsic since it does not uniquely represent a point in the circle. It is obvious to see that  $\tau$ and $\alpha \tau$ both represent the same point in the circle when scalar $\alpha>0$.
>

# Intrinsic Parameterizations for Parametric families
------
The examples in the previous section clearly show the importance of parameterization, and that it should be chosen such that the parameter space is an open set. Now, we discuss how to choose such a parameterization for a given parametric family.

Roughly speaking, a parameterization `$ \tau $` for a family of distribution `$ p(w|\tau) $` is intrinsic if the distribution `$ p(w|\tau) $` is smooth w.r.t. `$ \tau $` for all fixed `$ w $`.
Below is a formal condition.
...
Recall that an intrinsic parameterization must satisfy the conditions in openness, smoothness, and uniqueness.
These partial derivatives implies the smooth condition and the linear independence implies the uniqueness condition. 
In other words, an intrinsic parameterization is still necessary, just that the requirements are not explicit and hidden within the regularity condition.
We will discuss more about this at [here](#caveats-of-the-fisher-matrix-computation).


The following examples illustrates the regularity condition.

>Example 1 (regularity condition for an intrinsic parameterization): 
>
>We will write the regularity condition at a point for an intrinsic parameterization. Consider a 1-dimensional Gaussian family `$ \{ \mathcal{N}(w |\mu,\sigma) \Big| \mu \in \mathcal{R}, \sigma>0 \}$` with mean `$\mu$`, variance `$\sigma$`, and parametrization `$\tau = (\mu,\sigma) $`.
...
>$$`
> 
>Consider the partial derivatives at a point $(\mu=0, \sigma=1)$,
>
>`$$
>\begin{aligned}
...
\end{aligned}
>$$`
>For this point, the regularity condition will be `$ c_1 w + c_2  (\frac{ w^2 -1 }{2}) = 0$`. For this to be true for all $w$, we necessarily need to have `$c_1=c_2=0$`, which implies  linear independence. 
>
>A formal proof can be built to show that this holds for any `$\mu \in \mathcal{R}$` and `$\sigma >0$`.
...
Finally, recall that when we perform a parameter transformation, we require both the transformation map and its inverse map to be smooth.
