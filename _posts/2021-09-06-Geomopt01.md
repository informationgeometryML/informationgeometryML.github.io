...
>
>Consider a bivariate  Gaussian family with zero mean. There are many parametrizations.
>
> 1. Ill-defined paramtrization: `$ \Big\{ \exp\Big( -\frac{1}{2}\Big[  w^T \Sigma^{-1} w + \log \mathrm{det}(\Sigma) + 2 \log (2\pi)  \Big] \Big)  | \Sigma \in \mathcal{R}^{2 \times 2}   \Big\}$` since 
`$\log \mathrm{det}(\Sigma)$` must be well-defined. This parametrization leads to an ill-defined/incorrect FIM.
>
> 2. Ill-defined paramtrization: `$ \Big\{ \exp\Big( -\frac{1}{2}\Big[  w^T \Sigma^{-1} w + \log \mathrm{det}(\Sigma) + 2 \log (2\pi)  \Big] \Big)  | \mathrm{det}(\Sigma)>0, \Sigma \in \mathcal{R}^{2 \times 2}  \Big\}$` since `$ w^T \Sigma^{-1} w $` can be as negative as possible as $w \to \infty$ if $\Sigma^{-1}$ is not symmetric positive-definite.
> This parametrization leads to an ill-defined/incorrect FIM.
>
> 3. Well-defined paramtrization with **non-intrinsic**  $2$-by-$2$ (potentially asymmetric) parameter matrix `$\Sigma$`: `$\Big \{ \exp\Big( -\frac{1}{2}\Big[  w^T \big(\frac{ \Sigma^{-1} + \Sigma^{-T} }{2}\big) w + \log \mathrm{det}(\Sigma) + 2 \log (2\pi)  \Big] \Big)  |  \frac{ \Sigma^{-1} + \Sigma^{-T} }{2} \succ \mathbf{0},  \mathrm{det}(\Sigma)>0, \Sigma \in \mathcal{R}^{2 \times 2} \Big\}$`, where we have to explicitly enforce the symmetric constraint such as $w^T \big(\frac{ \Sigma^{-1} + \Sigma^{-T} }{2}\big) w $.
> This parametrization leads to a singular FIM.
>
> 4. Well-defined paramtrization with **non-intrinsic**  $2$-by-$2$ (potentially asymmetric) parameter matrix `$\Sigma$`: `$\Big \{ \exp\Big( -\frac{1}{2}\Big[  w^T \big(\frac{ \Sigma^{-1} + \Sigma^{-T} }{2}\big) w + \log \mathrm{det}\big(\frac{ \Sigma^{-1} + \Sigma^{-T} }{2}\big) + 2 \log (2\pi)  \Big] \Big)  |  \frac{ \Sigma^{-1} + \Sigma^{-T} }{2} \succ \mathbf{0},  \Sigma \in \mathcal{R}^{2 \times 2} \Big\}$`, where we have to explicitly enforce the symmetric constraint.
> This parametrization also leads to a singular FIM.
>
>
> 5. Well-defined paramtrization with **intrinsic** $3$-by-$1$  parameter vector `v`: `$\Big\{ \exp\Big( -\frac{1}{2}\Big[  w^T  ( \mathrm{vech}^{-1}(v))^{-1} w + \log \mathrm{det}( \mathrm{vech}^{-1}(v)) + 2 \log (2\pi)  \Big] \Big)  |   \mathrm{vech}^{-1}(v) \succ \mathbf{0} \Big\}$`.  Equivalentlly, parameter $\Sigma:=\mathrm{vech}^{-1}(v)$ is a symmetric parameter matrix.
