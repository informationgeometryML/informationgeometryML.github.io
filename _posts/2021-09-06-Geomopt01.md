...
We require that a manifold should be locally like a vector space denoted by `$ E $` under a proper  parameterization. Informally, we refer to 
...
We refer to this kind of parametrizations as an intrinsic parameterization[^1].
...
The matrix form is also known as the **Fisher information matrix** (FIM). Obviously, the form of the FIM depends on the choice of parameterizations. In many cases,  we could also compute the FIM as
