...
* better understand models and algorithms through the lens of differential geometry, information geometry, and invariant theory {% cite  liang2019fisher %}
...
>
> Algorithms such as  **Fisher scoring** and **(emprical) natural-gradient descent** {% cite   martens2020new %} are commonly used methods that exploit the geometric structure of `$p(x,y | \tau)$`.
...
This structure is also known as a manifold structure {% cite lee2013introduction %}. A (smooth) manifold should be locally like a "flat" vector space. For example, consider the (curved) Earth surface. It looks like a (locally) flat space to us. 
...
It supports local **vector additions** and local **real scalar products** (see [Part II]({{ site.baseurl }}{% post_url 2021-10-04-Geomopt02 %}#riemannian-gradients-as-tangent-vectors-optional) for the details.) 
</div>
...
# Fisher-Rao Metric
------
As we can see from the [previous section](#motivation), we  use the Fisher-Rao metric to design fast algorithms.
...
These statistical principles {%cite casella2002statistical %} play essential roles in training and estimating machine learning models. In other words,
the Fisher-Rao metric is special.
...
It is possible to define the FIM under a non-intrinstic parameterization. However, the FIM often is singular or ill-defined under a non-intrinstic  parameterization as shown below.
...
>Example 3  (Valid FIM):
>
>Consider Bernoulli family `$ \{ \mathcal{I}(w=0) \pi + \mathcal{I}(w=1) (1-\pi)  \Big| 0<\pi<1 \}$` with parameter `$\tau = \pi $`.
>
...
Mathematically speaking, we can show that any intrinsic parametrization of a manifold has the same degrees of freedom {% cite lee2013introduction %}.
This also gives us a tool to help identify non-manifold examples.
...
However, a closed
unit ball is NOT a manifold since its interior is a unit circle and its boundary is an open unit ball.
...
The following example illustrates these two maps.
...
> $$`
>
> The output of the  map `$\mathrm{MatH}(\mathbf{v})$` is
> `$$
