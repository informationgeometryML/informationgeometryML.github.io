...
:-------------------------:|:-------------------------:
 `$ \{ \mathcal{N}(w |0,\sigma) \Big|  \sigma>0 \}$` <br /> under intrinsic parameterization `$\tau = \sigma $` | `$ \{ \mathcal{N}(\mathbf{w} |\mathbf{0},\mathbf{\Sigma}) \Big| \mathrm{MatH}(\tau) = \mathbf{\Sigma}   \succ \mathbf{0} \}$`  <br /> under intrinsic parameterization `$\tau = \mathrm{vech}(\mathbf{\Sigma})$` is  a $\frac{d(d+1)}{2}$-dim array.  <br />  Map $\mathrm{MatH}()$ is the inverse map of <br /> the [half-vectorization function](https://en.wikipedia.org/wiki/Vectorization_(mathematics)#Half-vectorization) `$\mathrm{vech}()$`. <br />   `$\mathrm{vech}(\mathbf{\Sigma})$` is a `$\frac{d(d + 1)}{2}$`-dim array <br />  obtained by vectorizing only the lower triangular part of (symmetric) `$\mathbf{\Sigma}$`.
