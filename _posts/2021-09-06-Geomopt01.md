...
Note that `$\log \mathcal{N}(\mathbf{w} |\mathbf{0},\mathbf{S}^{-1}) := -\frac{1}{2} (d \log (2\pi) - \log \mathrm{det}( \frac{ \mathbf{S} + \mathbf{S}^T }{2} ) +\frac{1}{2} \mathbf{w}^T (\mathbf{S} + \mathbf{S}^T ) \mathbf{w} ) $`, where we have to explicitly enforce the symmetric constraint during the Fisher matrix computation.
We  can use 
