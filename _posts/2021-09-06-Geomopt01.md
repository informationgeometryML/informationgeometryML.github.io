...
For example, let's consider a Gaussian family.
...
Note that this regularity condition implicitly assumes that the parameter space $\Omega_\tau$ is an open set in `$\mathcal{R}^K$`, where K is the number of entries in parameter vector `$\tau$`.
We will discuss more about this at [here](#caveats-of-the-fisher-matrix-computation).


We will use hexamples to illustrate this condition.
