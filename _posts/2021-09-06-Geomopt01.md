...
when we specify these two parametrizations.
...
>
>This parametrization is not intrinsic since it does not uniquely represent a point in the circle. It is obvious to see that  $\tau_0$ and $\alpha \tau_0$ both represent the same point in the circle when scalar $\alpha>0$.
>
...
Roughly speaking, a parameterization `$ \tau $` for a family of distribution `$ p(w|\tau) $` is intrinsic if the distribution `$ p(w|\tau) $` is smooth and unique w.r.t. `$ \tau $` where `$ w $` should be considered to be fixed and arbitrary.
...
`$ \{ \partial_{\tau_i} \log p(w|\tau) \} $`  should be linearly independent for (almost) all `$w$`.
...
> Clearly, when $w=0$, they are linearly dependent. However, for almost all $w$, we necessarily need to have `$c_1=c_2=0$`, which implies linear independence. 
>
