...
In other words, we should allow a small perturbation $ E $ such as an open ball in a given parameter space. A technical assumption of  parameterization $\tau$ is that the parameter space denoted by $\Omega_\tau$ should be an open set in `$\mathcal{R}^k$`, where $k$ is the number of parameters.
