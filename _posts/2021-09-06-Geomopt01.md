...
Note that `$\log \mathcal{N}(\mathbf{w} |\mathbf{0},\mathbf{S}^{-1}) := -\frac{1}{2} (d \log (2\pi) - \log \mathrm{det}(  \mathbf{S} ) +\frac{1}{2} \mathbf{w}^T (\mathbf{S} + \mathbf{S}^T ) \mathbf{w} ) $`, where we explicitly enforce the symmetric constraint.
We  can use 
