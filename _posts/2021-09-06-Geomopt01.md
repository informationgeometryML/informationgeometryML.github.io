...
>
> Algorithms such as  [**Fisher scoring**](https://en.wikipedia.org/wiki/Scoring_algorithm#Fisher_scoring)  and **(emprical) natural-gradient descent** {% cite   martens2020new %} are commonly used methods that exploit the geometric structure of `$p(x,y | \tau)$`.  These algorithms can be generalized to neural network cases {% cite   martens2020new %} from a frequentist viewpoint.
...
>
>The **natural-gradient variational inference** {% cite khan2017conjugate  %} is an algorithm that speeds up the inference by exploiting the geometry of `$q(z|\tau)$` induced by the Fisher-Rao metric. This approach can be generalized to neural network cases {% cite lin2021tractable %} {% cite osawa2019practical %}  from a Bayesian viewpoint.
