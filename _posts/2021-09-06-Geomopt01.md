...
Roughly speaking, a parameterization `$ \tau $` for a family of distribution `$ p(w|\tau) $` is intrinsic if the distribution `$ p(w|\tau) $` is smooth and unique w.r.t. `$ \tau $` where `$ w $` should be considered to be fixed but arbitrary.
