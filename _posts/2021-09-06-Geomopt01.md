...
>
> Algorithms such as  [**Fisher scoring**](https://en.wikipedia.org/wiki/Scoring_algorithm#Fisher_scoring)  and **(emprical) natural-gradient descent** {% cite   martens2020new %} are commonly used methods that exploit the geometric structure of `$p(x,y | \tau)$`.  These algorithms can be generalized to neural network cases {% cite   martens2020new %}.
