...
>
> 3. well-defined paramtrization with non-intrinsic  $2$-by-$2$ (asymmetric) parameter matrix `$\Sigma$`: `$\Big \{ \exp\Big( -\frac{1}{2}\Big[  w^T \big(\frac{ \Sigma^{-1} + \Sigma^{-T} }{2}\big) w + \log \mathrm{det}(\Sigma) + 2 \log (2\pi)  \Big] \Big)  |  \frac{ \Sigma^{-1} + \Sigma^{-T} }{2} \succ \mathbf{0},  \mathrm{det}(\Sigma)>0, \Sigma \in \mathcal{R}^{2 \times 2} \Big\}$`, where we have to explicitly enforce the symmetric constraint such as $w^T \big(\frac{ \Sigma^{-1} + \Sigma^{-T} }{2}\big) w $.
