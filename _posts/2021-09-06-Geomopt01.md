...
p(w|\tau) $` is smooth and unique w.r.t. `$ \tau $` in its parameter space `$\Omega_\tau$`.
Below is the formal condition.

<div class="notice--success" markdown="1">
**Regularity Condition**: 

For any `$\tau \in \Omega_\tau$`,  the smooth partial derivatives 
`$ \{ \partial_{\tau_i} \log p(w|\tau) \} $`  is a set of linearly independent functions of `$w$`.
