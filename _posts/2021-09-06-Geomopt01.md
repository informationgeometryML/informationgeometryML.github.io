...
>
> 2. Ill-defined paramtrization: `$ \Big\{ \exp\Big( -\frac{1}{2}\Big[  w^T \Sigma^{-1} w + \log \mathrm{det}(\Sigma) + 2 \log (2\pi)  \Big] \Big)  \Big| \mathrm{det}(\Sigma)>0, \Sigma \in \mathcal{R}^{2 \times 2}  \Big\}$` since `$ - w^T \Sigma^{-1} w $` can be as large as possible as $||w||_2 \to \infty$ if $\Sigma^{-1}$ is not symmetric positive-definite.
