...
* We also require that `$ E $`  **uniquely** and **smoothly** represents a sub-set of points in a manifold. The condition ensures arbitrary (smooth) parameter transformations should still represent the same sub-set. In other words, we require that there exists **bi-jective** maps among such parameterizations and that the maps and their inverse maps are both **smooth**. In differential geometry, this  requirement is known as a diffeomorphism, which is a formal but more abstract definition of this requirement.
