...
>More Example   (Ill-defined FIM and Singular FIM):
>
...
`$\log \mathrm{det}(\Sigma)$` must be well-defined. This parametrization leads to an ill-defined FIM.
>
...
> This parametrization leads to an ill-defined FIM.
>
...
>
>
> 4. Well-defined paramtrization with **intrinsic** $3$-by-$1$  parameter vector `v`: `$\Big\{ \exp\Big( -\frac{1}{2}\Big[  w^T  ( \mathrm{vech}^{-1}(v))^{-1} w + \log \mathrm{det}( \mathrm{vech}^{-1}(v)) + 2 \log (2\pi)  \Big] \Big)  |   \mathrm{vech}^{-1}(v) \succ \mathbf{0} \Big\}$`.  Equivalentlly, parameter $\Sigma:=\mathrm{vech}^{-1}(v)$ is a symmetric parameter matrix.
