...
Roughly speaking, a parameterization `$ \tau $` for a family of distribution `$ p(w|\tau) $` is intrinsic if the distribution `$ p(w|\tau) $` is smooth and unique w.r.t. `$ \tau $` for all fixed `$ w $`.
Below is a formal condition.

<div class="notice--success" markdown="1">
**Regularity Condition**: 

The following continuous partial derivatives 
