...
Goal
------
This blog post focuses on the Fisher-Rao metric, also known as the Fisher information matrix (FIM) under a parametrization. The purpose is to show that
* The FIM plays an essential role in statistics and machine learning 
* For a parametric distribution family, the FIM induces a **Riemannian** geometric-structure 
...
This structure is also known as a manifold structure {% cite tu2011introduction  %}. A (smooth) manifold should be locally like a "flat" vector space. For example, consider the (curved) Earth surface. It looks like a (locally) flat space to us. In differential geometry, a proper parametrization is call an intrinsic parameterization[^1].
