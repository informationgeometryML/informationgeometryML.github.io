...
the Riemannian steepest direction with this metric is 
...
\end{aligned}
$$` This Riemannian steepest direction is exactly the normalized gradient direction in Newton's method, where we use the weighted inner product to define the norm.
...
As we will discuss more in Part V, computing Riemannians gradient with this Newton  metric are often more computaitonally intensive compared to the Fisher-Rao metric. Moreover, Riemannian gradients with this Newton  metric are dependent on the convexity of the loss function (see more about this in Part V).
