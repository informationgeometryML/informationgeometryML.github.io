...
The key observation is that we can use the Hessian to define a new Riemannian metric `$\hat{\mathbf{F}}_\tau:=\nabla_\tau^2 f(\mathbf{\tau})$` under parametrization $\tau$. Note that  `$\hat{\mathbf{F}}_\tau$` in general is not the same as the Fisher-Rao metric `${\mathbf{F}}_\tau$` . 
...
In this case, using the result of [Riemannian steepest direction]({{ site.baseurl }}{% post_url 2021-10-04-Geomopt02 %}#riemannian-steepest-direction), it is easy to show that
...
= = -\frac{  [\nabla_\tau^2 f_\tau(\mathbf{\tau})]^{-1} \nabla_\tau f(\mathbf{\tau}_0) }{\| [\nabla_\tau^2 f_\tau(\mathbf{\tau})]^{-1} (\mathbf{\tau}_0)\nabla_\tau f(\mathbf{\tau}_0) \|_\hat{F}} 
...
Readers may wonder why use the Newton metric instead of the Fisher-Rao metric.  Key reasons are the Fisher-Rao metric is defined via the manifold while this Newton metric is defined via the loss function.
As we will discuss more in Part V, computing Riemannians gradient with this Newton's metric are often more computaitonally intensive compared to the Fisher-Rao metric. Moreover,  Riemannians gradient with this Newton's metric are dependent on the choise of a loss function (see more about this in Part V).
