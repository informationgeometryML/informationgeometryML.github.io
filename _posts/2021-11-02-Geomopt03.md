...
We will also discuss why the Euclidean steepest direction is NOT invariant while the gradient direction in Newton's method is invariant.
...
In this blog post, we show that two key **geometric properties** remains the same under an **intrinsic** parameter transformation.
...
# Gradient Direction in Newton's Method is Invariant
------
Let's consider the following optimization problem  `$\min_{\tau \in \Omega_\tau } f_\tau(\mathbf{\tau})$` under an intrinsic parameterization $\mathbf{\tau}$.

For Newton's method, we have to assume the loss function `$f(\mathbf{\tau})$` to be stronly convex so that the Hessian `$\nabla_\tau^2 f_\tau(\mathbf{\tau})$` is positive-definite everywhere in `$\Omega_\tau$`.

The key observation is that we can use the Hessian to define a new Riemannian metric `$\hat{\mathbf{F}}_\tau:=\nabla_\tau^2 f_\tau(\mathbf{\tau})$` under parametrization $\tau$. Note that  `$\hat{\mathbf{F}}_\tau$` in general is not the same as the Fisher-Rao metric `${\mathbf{F}}_\tau$` . 

We can show that the new metric `$\hat{\mathbf{F}}_\tau$` follows the same parameter transformation rule as the Fisher-Rao metric as shown below.
`$$
\begin{aligned}
\hat{\mathbf{F}}_{\tau} (\tau_0) = \underbrace{\mathbf{J}^T}_{  \frac{ \partial \lambda^i(\tau_0) }{ \partial \tau^k }  } \hat{\mathbf{F}}_{\lambda} (\lambda_0) \underbrace{\mathbf{J}}_{  \frac{ \partial \lambda^l(\tau_0) }{ \partial \tau^j }  } 
\end{aligned}
$$` where `$\hat{\mathbf{F}}_\tau :=\nabla_\tau^2 f_\tau(\mathbf{\tau}) =\nabla_\tau^2 [ f_\lambda({\lambda}(\tau)) ]$` and $f_\tau(\mathbf{\tau}) =  f_\lambda({\lambda}(\tau))$.

In this case, using the result of the [Riemannian steepest direction]({{ site.baseurl }}{% post_url 2021-10-04-Geomopt02 %}#riemannian-steepest-direction), it is easy to show that
the Riemannian steepest direction under this metric is 

`$$
\mathbf{v}_{\text{opt}}= -\frac{ \hat{\mathbf{F}}_\tau^{-1}(\mathbf{\tau}_0) \nabla_\tau f(\mathbf{\tau}_0) }{\| \hat{\mathbf{F}}_\tau^{-1}(\mathbf{\tau}_0)\nabla_\tau f(\mathbf{\tau}_0) \|_\hat{F}}
= = -\frac{  [\nabla_\tau^2 f_\tau(\tau_0)]^{-1} \nabla_\tau f(\mathbf{\tau}_0) }{\| [\nabla_\tau^2 f_\tau(\tau_0)]^{-1} (\mathbf{\tau}_0)\nabla_\tau f(\mathbf{\tau}_0) \|_\hat{F}} 
$$` which is exactly the normalized gradient direction in Newton's method. 

It is obvious that the gradient direction in Newton's method is invariant since it is a speical case of the Riemannian steepest direction.
This invariance property also implies that Newton's method is linearly invariant.
 

Readers may wonder why not use the Newton metric instead of the Fisher-Rao metric.  Key reasons are the Fisher-Rao metric is defined via the manifold while this Newton metric is defined via the loss function.
As we will discuss more in Part V, computing Riemannians gradient with this Newton  metric are often more computaitonally intensive compared to the Fisher-Rao metric. Moreover, Riemannians gradient with this Newton  metric are dependent on the choice of a loss function (see more about this in Part V).
