...
By Eq `$\eqref{3}$`, we obtain the transformation rule for Riemannian gradients as below,  where `$\mathbf{Q}:=\mathbf{J}^{-1}$` is also a Jacobian matrix and `$Q_{ki}=\frac{\partial \tau^k(\lambda)}{\partial \lambda^i}$` is the `$(k,i)$` entry of matrix `$\mathbf{Q}$`.
...
Note:
<span style="color:blue">**column**</span> vector `$\hat{\mathbf{g}}_\tau$` can be computed via a Jacobian-vector product used in forward-mode differentiation {% cite salimbeni2018natural %} given that `$\hat{\mathbf{g}}_\lambda$` is pre-computed.
</div>
