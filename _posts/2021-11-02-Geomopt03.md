...
|   Type of  directions  |   transformation rules  |
|:------------|:-------------:|
| Euclidean gradient |  `$\eqref{6}$`  | 
| Euclidean steepest descent direction (normalized Euclidean gradient)  |  N/A  |   
| Riemannian gradient |  `$\eqref{7}$` | 
|  Riemannian steepest descent direction (normalized Riemannian gradient) |  `$\eqref{7}$`  |   
...
## Transformation Rules for Natural Gradients and Euclidean Gradients 
...
\end{aligned}
$$` where we consider $t$ to be fixed. 
...
Note: <span style="color:red">**row**</span> vector `${\mathbf{g}}_\tau^T$` can be computed via a vector-Jacobian product in any standard Auto-Diff toolbox given that `${\mathbf{g}}_\lambda$` is pre-computed.
...
Note: <span style="color:red">**column**</span> vector `$\hat{\mathbf{g}}_\tau$` can be computed via a Jacobian-vector product used in forward-mode differentiation given that `$\hat{\mathbf{g}}_\lambda$` is pre-computed.
...
Now, we give a transformation rule for the Fisher information matrix as defined at [Part I]({{ site.baseurl }}{% post_url 2021-09-06-Geomopt01 %}#fisher-rao-metric).
...
As an example, consider the following example discussed in 
...
> `$$
> \begin{aligned}
> f(\tau):= E_{q(w|\tau)} [ w^2 ] + \frac{1}{2} \log(s)
> = \mu^2 + \frac{1}{s} + \frac{1}{2} \log(s)
> \end{aligned}
> $$`
...
>$$`
>
> Now, consider the following re-parametrization of the function, where 
>  $v=\frac{1}{s}$.
>
> `$$
> \begin{aligned}
> f(\lambda):= E_{q(w|\lambda)} [ w^2 + \log q(w|\lambda)]
> = \mu^2 + v - \frac{1}{2} \log(v) - \frac{1}{2}(1+\log(2\pi))
> \end{aligned}
> $$`
> where `$q(w|\lambda)= \mathcal{N}(w|\mu,v)$` with mean `$\mu$` and variance `$v$`,
>   intrinsic parametrization `$\lambda=(\mu,v)$`, and parameter space `$\Omega_\lambda=\{(\mu,v)|\mu \in \mathcal{R},v>0 \}$`.
>
> The Fisher information matrix of Gaussian $q(w|\lambda)$ under this parametrization is
> `$$
> \begin{aligned}
...
We can easily to verify the following identities since
