...
By the (standard) chain rule for a Euclidean gradient[^1], we has
`$$
\begin{aligned}
...
## Footnotes:
[^1]: We assume readers are familar with the transformation rule for Euclidean gradients. In differential geometry, this transformation rule can also be shown by using an abstract coordinate map. We avoid defining this map for simplicity.
