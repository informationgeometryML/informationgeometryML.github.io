...
Consider continous map `$\mathbf{m}(\eta):=\nabla_\eta A_\eta(\eta)$`, where its domain is `$\Omega_\eta$`.

Since `$\nabla_\eta^2 A_\eta(\eta)$` is positive-definite in open set `$\Omega_\eta$`, we can show that there exists an one-to-one map between the
natural parameter `$\eta$` and the expectation parameter `$\mathbf{m}$`, which implies that map `$\mathbf{m}(\cdot)$` is injective.

We define the domain of the expectation parameter space as `$\Omega_m :=\{\mathbf{m}(\eta) | \eta \in \Omega_\eta \}$`.

Since `$\Omega_\eta$` is open in `$\mathcal{R}^K$`, we can show that
the expectation parameter space `$\Omega_m$` is also open in `$\mathcal{R}^K$` due to the invariance of domain.
...
\end{aligned}
$$` where `$\mathrm{B}_\Phi(\cdot,\cdot)$` is a Bregman divergence  equipped pairred with the convex function `$\Phi(\cdot)$`.
...
Since
`$\Omega_m=\mathcal{R}^k$` is unconstrained, the optimal expectation parameter `$\mathbf{m}_{k+1}=\bar{\mathbf{x}}$`
must satisfy the stationary condition as

`$$
\begin{aligned}
\mathbf{0} &= \nabla_x u(\bar{\mathbf{x}}) & (\mathbf{m}_k \text{ is considered to be a constant}) \\
...
Therefore,  mirror descent of `$\eqref{4}$` in **expectation** parameter space `$\Omega_m=\mathcal{R}^K$` gives 
