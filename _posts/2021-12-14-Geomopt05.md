...
Moreover, we have the following identity since by `$\eqref{4}$`, `$A_\eta(\eta)=\langle \mathbf{\eta},\mathbf{m} \rangle- A^*_\eta( \mathbf{m}) $`.
`$$
\begin{aligned}
...
The stationary point of `$\eqref{5}$`,  `$\hat{\mathbf{x}}$`,  must satisfy the following
 condition.
`$$
\begin{aligned}
...
We frist show that the stationary point is attainable even with the parameter constraint
`$\Omega_m$`.
...
We can show that
the stationary point `$\hat{\mathbf{x}} = \nabla_\eta A_\eta( \mathbf{\eta}_{k+1})$` by convex bi-conjugacy.
Since  `$\mathbf{\eta}_{k+1} \in \Omega_\eta$`, we have
`$\hat{\mathbf{x}} \in \Omega_m$` due to  the definition of `$\Omega_m$`.



Moreover,
the stationary point is the unique  solution of `$\eqref{5}$` since `$\nabla_x^2 u(\mathbf{x}) =\nabla_x^2 A^*_\eta(
{\mathbf{x}})$` is positive-definite in `$\Omega_m$` and therefore strictly convex.


In summary, when `$\Omega_\eta = \mathcal{R}^K$`, the unique solution of `$\eqref{5}$` even in a constrained space
`$\Omega_m$` is 
`$$
\begin{aligned}
...
In other words,
mirror descent of `$\eqref{5}$` in **expectation** parameter space `$\Omega_m$` is equivalent to
...
descent in **natural** parameter space `$\Omega_\eta=\mathcal{R}^K$` since by `$\eqref{3}$`, we have `$\nabla_m \ell(\mathbf{m}_k) = \nabla_m f( \eta_k)= \mathbf{F}
_\eta^{-1} (\eta_k) \nabla_\eta f(\eta_k)$`.
