...
> $\Sigma$ is considered as a d-by-d matrix with `$\frac{d(d+1)}{2}$` distinct entries.
>
>
...
> \frac{\partial \ell}{\partial \mathbf{V} } & =\mathbf{g}_{V} \,\,\,\, ( \mathbf{V}=\Sigma \text{ is a d-by-d matrix with } d^2  \text{ distinct entries} ) \\
...
> `$$
> \begin{aligned}
> \frac{\partial \ell}{\partial m^{(1)}} &= \mathbf{g}_\mu^T - 2 (\mathbf{m}^{(1)})^T \mathbf{g}_{V}^T \\
> \frac{\partial \ell}{\partial m^{(2)}} &= \mathbf{0} + ( \mathrm{vech}(\mathbf{g}_{V}))^T
> \end{aligned}
> $$` 
</fieldset>
</details>
