...
the expectation parameter space `$\Omega_m$` is also open in `$\mathcal{R}^K$` due to the [invariance of domain](https://en.wikipedia.org/wiki/Invariance_of_domain)[^1].
...
## Footnotes:
[^1]: Indeed, this Legendre transformation is diffeomorphic since we assume the natural parameter `$\eta$` is minimal and therefore, `$\nabla_\eta^2 A_\eta(\eta)$` is positive-definite.
