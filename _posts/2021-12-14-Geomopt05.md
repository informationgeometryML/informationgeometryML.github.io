...
We say natural parametrization `$\eta$` is **regular** if parameter space `$\Omega_\eta$` is  relatively **open**.
...
We say a natural parametrization is **minimal** if the sufficient stattistics `$\mathcal{T}(\mathbf{w})$` is linearly independent.
...
>\end{aligned}
> $$` which implies that
the sufficient stattistics `$\mathcal{T}(\mathbf{w})$` is linearly dependent. This is a contradiction.
...
# Natural-gradient Methods for Gaussians as Newton-like Updates
------
