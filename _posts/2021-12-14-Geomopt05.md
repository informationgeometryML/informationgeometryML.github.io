...
Readers should be aware of the following points when using an expoential family.
...
Now, we discuss particular parametrizations of an expoential family. We could efficiently compute natural-gradients when
...
Moreover, the FIM of the expoential family under parametrization `$\eta$` is
`$$
\begin{aligned}
...
Since the Legendre transformation is defined in `$\Omega_\eta$` , which implies the dual (expectation) parameter space `$\Omega_m$` is constrained.

to do: add a Gaussian example
...
>
> By the definition of (constrained) mirror descent in `$\eqref{4}$`, the expectation parameter must satisify `$\mathbf{m}_{k+1} \in \Omega_m$`.
>
> Therefore, the corresponding natural parameter must satisify `$\mathbf{\eta}_{k+1} \in \Omega_\eta$`.
>
...
the [weighted inner product]({{ site.baseurl }}{% post_url 2021-10-04-Geomopt02 %}#distance-induced-by-the-fisher-rao-metric) with the FIM highlighted in red.


On the other hand, the constrained mirror descent remains the same as in  `$\eqref{4}$`. 
`$$
\begin{aligned}
\mathbf{m}_{k+1} \leftarrow \arg \min_{x \in \Omega_m}\{ \langle \nabla_m \ell(\mathbf{m}_k), \mathbf{x}-\mathbf{m}_k  \rangle + \frac{1}{\alpha}  \mathrm{B}_{A^*_\eta}(\mathbf{x},\mathbf{m}_k) \}
\end{aligned}
...
These two methods could be very difficult to solve since `$\Omega_m$` can be an arbitrary open subset in `$\mathcal{R}^K$`.
...
Therefore, one idea is to use an adaptive step-size to satisify the parameter constraint at each iteration.
`$$
\begin{aligned}
...
However, for a general parameter constraint `$\Omega_m$`, this approach will often select an extremally small step-size
`$\alpha_k$`,
...
To avoid solving the geodeisc ODE to get the manifold expoential map, we could use an (inexact) approximation, which
...
However, for a general parameter constraint `$\Omega_m$`, it can be difficult to come out an efficient retraction map to satisify
the constraint.
