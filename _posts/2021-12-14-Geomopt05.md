...
## NGD in the Unconstrained Natural Space as MD in the Expectation Space
...
Note that  the expectation space `$\Omega_m$` may be constrained.

to do: add an example

Recall that the convex conjugate of the log-partition function `$A^*_\eta$` is strcitly convex in the expectation parameter space `$\Omega_m$`.
Now, consider the following mirror descent in `$\Omega_m$` as
...
The stationary point `$\hat{\mathbf{x}}$` of `$\eqref{4}$` must satisfy the following
 condition.
`$$
\begin{aligned}
...
Let's denote `$\mathbf{\eta}_{k+1}:=\nabla_x A^*_\eta( \hat{\mathbf{x}})= \eta_k - \alpha \nabla_m \ell(\mathbf{m}_k)$`. Since `$\Omega_\eta$` is unconstrained, it
is obvious that `$\mathbf{\eta}_{k+1} \in \Omega_\eta$`. Therefore, the 
stationary point `$\hat{\mathbf{x}} \in \Omega_m$` is attainable.
Since `$\nabla_x^2 u(\mathbf{x}) =\nabla_x^2 A^*_\eta(
{\mathbf{x}})$` is positive-definite  in `$\Omega_m$`, we know that the optimal solution 
of `$\eqref{4}$` must be `$\mathbf{m}_{k+1} = \hat{\mathbf{x}}$`.
In other words, `$\mathbf{\eta}_{k+1}=\nabla_x A^*_\eta( \mathbf{m}_{k+1})$`.
Using the duality, we also have 
`$\mathbf{m}_{k+1}=\nabla_x A_\eta( \mathbf{\eta}_{k+1})$`, where 
`$\eta_{k+1}  = \eta_k - \alpha \nabla_m \ell(\mathbf{m}_k)$`.

In summary, when `$\Omega_\eta = \mathcal{R}^K$`, the optimal solution of `$\eqref{4}$` even under constraint
`$\Omega_m$` is 
`$$
\begin{aligned}
\eta_{k+1} & = \eta_k - \alpha \nabla_m \ell(\mathbf{m}_k) \\
...
In other words,
mirror descent of `$\eqref{4}$` in **expectation** parameter space `$\Omega_m$` is equivalent to
