...
## Footnotes:
[^1]: Indeed, this Legendre transformation is diffeomorphic since we assume the natural parameter `$\eta$` is minimaland therefore, `$\nabla_\eta^2 A_\eta(\eta)$` is positive-definite. In other words, the expectation parameter `$\mathbf{m}$` is also intrinsic when the natural parameter `$\eta$` is minimal.
