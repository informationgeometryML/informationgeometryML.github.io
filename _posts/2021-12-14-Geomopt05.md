...
We first show that the stationary point is attainable.
...
Now, we show that the stationary point is the optimal solution of `$\eqref{5}$`.
Since `$\nabla_x^2 u(\mathbf{x}) =\nabla_x^2 A^*_\eta(
{\mathbf{x}})$` is positive-definite in `$\Omega_m$`, we know that the optimal solution 
of `$\eqref{5}$` must be the stationary point as `$\mathbf{m}_{k+1} = \hat{\mathbf{x}} $`.

In summary, when `$\Omega_\eta = \mathcal{R}^K$`, the optimal solution of `$\eqref{5}$` even under constraint
`$\Omega_m$` is 
`$$
\begin{aligned}
