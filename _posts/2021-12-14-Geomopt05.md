...
* The log-partition function can be differentiable w.r.t. `$\eta$` even when `$\mathbf{w}$` is discrete.
    <div class="notice--info" markdown="1">
    <details>
...
**Claim**: A regular, minimal, and natural parametrization is intrinstic.
<div class="notice--info" markdown="1">
<details>
...
>
>
> Suppose a regular, minimal, and natural parametrization is not intrinstic, 
> these partial derivatives must linearly dependent.
...
> We define a new map `$\mathrm{vec2h}(\mathrm{S}):=\mathrm{vech}\big(2\mathrm{S} - \mathrm{Diag}( \mathrm{diag}(\mathrm{S}) ) \big)$`, which is like map $\mathrm{vech}()$ except the off-diagonal entries in the lower-traingular part are multiplied by 2.
>
> A minimal natural parametrization $\eta$ should be defined as
>`$$
>\begin{aligned}
>p({w}|\mathbf{\eta})
>&= \underbrace{ \exp(0) }_{  h_\eta({w}) }  \exp( \langle \underbrace{\begin{bmatrix} -\frac{1}{2} \mathrm{vech}( \Sigma^{-1} ) \\ \Sigma^{-1}\mu  \end{bmatrix}}_{\mathbf{\eta} }  ,  \underbrace{\begin{bmatrix}  \mathrm{vec2h}( \mathrm{w}\mathbf{w}^T )   ) \\ \mathbf{w}  \end{bmatrix}}_{ \mathbf{T}_\eta ({w}) } \rangle  -   \frac{1}{2} [ d\log ( 2\pi ) + \log \mathrm{det} (\Sigma) + \mu^T \Sigma^{-1} \mu ]     )   \\
>\end{aligned}
...
Now, we show how to compute natural-gradients .

**Claim**: We can efficiently   compute natural-gradients 
in $O( d^3 )$ iteration cost
...
>
> By using the result in this section, we can efficiently  compute each natural gradient w.r.t. $\eta$ by the corresponding computing Eucldiean gradient w.r.t. $\mathbf{m}$. 
>
...
> \frac{\partial \ell}{\partial \Sigma} & =\mathbf{g}_{\Sigma} \,\,\,\, (\Sigma \text{ has } \frac{d(d+1)}{2}  \text{ dinstinct entries} ) \\
> \frac{\partial \ell}{\partial \mathrm{vec2h}(\Sigma)} &=\mathbf{g}^T_{\mathrm{vec2h}(\Sigma)} = ( \mathrm{vech}(\mathbf{g}_{\Sigma}))^T 
> \end{aligned}
> $$` 
> We can verify the following identity for `$\mathbf{m}^{(1)}$`.
> `$$
> \begin{aligned}
> \frac{\partial \ell}{\partial \mathrm{vec2h}(\Sigma)} \frac{\partial \mathrm{vec2h}(\Sigma) }{\partial m^{(1)}_i} &=   \mathrm{Trace}\big(\frac{\partial \ell}{\partial \Sigma} \frac{\partial \Sigma }{\partial m^{(1)}_i} \big)
> \end{aligned}
> $$`  where `$\Sigma= \mathbf{m}^{(2)} - \mathbf{m}^{(1)}(\mathbf{m}^{(1)})^T$`.
>
...
> `$$
> \begin{aligned}
> \frac{\partial \ell}{\partial m^{(1)}} &= \mathbf{g}_\mu^T - 2 (\mathbf{m}^{(1)})^T \mathbf{g}_{\Sigma}^T \\
> \frac{\partial \ell}{\partial m^{(2)}} &= \mathbf{0} +   \mathbf{g}_{\mathrm{vec2h}(\Sigma)}^T =( \mathrm{vech}(\mathbf{g}_{\Sigma}))^T
> \end{aligned}
> $$` 
</fieldset>
</details>
...
Note that we define `$A_\eta(\mathbf{x}):=+\infty$` when `$\mathbf{x} \not \in \Omega_\eta$`.
...
When `$\mathbf{m} \in \Omega_m$`, we have the following identity.
`$$
\begin{aligned}
...
**Claim**: when `$\Omega_\eta = \mathcal{R}^K$`, the solution of `$\eqref{7}$` is equivalent to `$\eta_{k+1}  \leftarrow  \eta_k - \alpha \nabla_m \ell(\mathbf{m}_k)$`.
