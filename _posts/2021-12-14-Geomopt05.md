...
> We define a new map `$\mathrm{vec2h}(\mathrm{S}):=\mathrm{vech}\big(2\mathrm{S} - \mathrm{Diag}( \mathrm{diag}(\mathrm{S}) ) \big)$`, which is like map $\mathrm{vech}()$ except the off-diagonal entries which are multiplied by 2.
>
...
> \frac{\partial \ell}{\partial \Sigma} & =\mathbf{g}_{\Sigma} \,\,\,\, (\Sigma \text{ is a d-by-d matrix with } \frac{d(d+1)}{2}  \text{ distinct entries} ) \\
