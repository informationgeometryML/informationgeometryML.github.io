...
a continous map `$\mathbf{m}(\eta):=\nabla_\eta A_\eta(\eta)$`, where the input domain is `$\Omega_\eta$`. 

We define the expectation parameter space as the output domain of the map `$\Omega_m :=\{\mathbf{m}(\eta) | \eta \in \Omega_\eta \}$`.
...
Consider the following optimization problem over a **closed** convex domain denoted by `$\Omega_\theta$`.
`$$
\begin{aligned}
...
\end{aligned}
$$` where `$\mathrm{B}_\Phi(\cdot,\cdot)$` is a Bregman divergence  equipped  with the strictlyy convex function `$\Phi(\cdot)$`.



## NGD as Unconstrained Mirror Descent in the Expectation Space
...
Note that we have shown that the convex conjugate of the log-partition function `$A^*_\eta$` is strcitly convex in the expectation parameter space `$\Omega_m$`. By our
assumption,  `$\Omega_{m_\eta}=\mathcal{R}^k$` is both open and closed in `$\mathcal{R}^k$`.


Therefore, we can consider the following mirror descent in the 
...
Since
`$\Omega_m=\mathcal{R}^k$` is unconstrained, the optimal expectation parameter `$\mathbf{m}_{k+1}$`
must satisfy the stationary condition as

`$$
\begin{aligned}
\mathbf{0} &= \nabla_x u({\mathbf{x}})\Big|_{x=m_{k+1}} & (\mathbf{m}_k \text{ is considered to be a constant}) \\
&= \nabla_m \ell(\mathbf{m}_k)+ \frac{1}{\alpha}  [\underbrace{ \nabla_x A^*_\eta( {\mathbf{m}_{k+1}})}_{\eta_{k+1}}  -  \eta_k ]
\end{aligned}
$$`
Note that as long as `$\eta_{k+1} \in \Omega_\eta$`, it is the corresponding natural parameter for the expectation
parameter `$\mathbf{m}_{k+1}$` since there exists an one-to-one relationship between them.

The above expression implies that the solution of `$\eqref{4}$` is
`$$
\begin{aligned}
...
$$`
Note that as long as `$\eta_{k+1} \in \Omega_\eta$`, `$\mathbf{m}_{k+1}$` will always stays in the constraint set
`$\Omega_m$` even when `$\Omega_m$` is constrained. This implies that even when `$\Omega_m$` is constrained,
(constrained) mirror
descent in `$\eqref{4}$` still recovers natural-gradient descent as long as 
`$\Omega_\eta$` is unconstrained.



In other words,
mirror descent of `$\eqref{4}$` in **expectation** parameter space `$\Omega_m=\mathcal{R}^K$` is equivalent to
...
descent in **natural** parameter space `$\Omega_\eta=\mathcal{R}^K$` since by `$\eqref{2}$`, we have `$\nabla_m \ell(\mathbf{m}_k) = \nabla_m f( \eta_k)= \mathbf{F}
_\eta^{-1} (\eta_k) \nabla_\eta f(\eta_k)$`.
